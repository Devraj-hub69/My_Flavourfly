...
let getDetails=JSON.parse(localStorage.getItem("DetailsFood"));
const fooddetails = document.getElementById("details");
function displayProducts(getDetails) {
        ...
        heading.innerText = getDetails.category || "No Category";

        let img = document.createElement("img");
        img.src = getDetails.image_url || "placeholder.jpg";


        let name = document.createElement("h3");
        name.innerText = `Varient:${getDetails.name}`;

        let price = document.createElement("p");
        price.innerText = `Price: $${getDetails.price}`;

        let rating = document.createElement("p");
        rating.innerText = `Rating: ${getDetails.rating || "N/A"}`;

        let ingredients = document.createElement("p");
        ingredients.innerHTML = `<strong>Ingredients:</strong> ${getDetails.ingredients.join(", ")}`;
